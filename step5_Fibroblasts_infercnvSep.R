...
for (i in datalist) {
  NPC.tmp <- subset(NPC.Fib, subset = orig.ident %in% c(i))
  NPC.tmp@meta.data$infercnv <- NPC.tmp@meta.data$Annotation.CNV
  ...
  ##############################################################
  # infercnv
  path.annotation = paste("G:/scRNA data/DS0/infercnv/",i,"_Fibroblast/annotations.txt",sep="")
  write.table(file_ann, file= path.annotation, row.names=FALSE, col.names=FALSE, quote =FALSE, sep ="\t")
  path.save = paste("G:/scRNA data/DS0/infercnv/",i,"_Fibroblast/counts_matrix",sep="")
  save(counts_matrix, file = path.save)
  
  #path.load = path.save
  #load(path.load)
  path.out <- paste("G:/scRNA data/DS0/infercnv/",i,"_Fibroblast",sep="")
